...
def pull_dataset(
    ...
    *,
    ...
    for view in bq_client.list_views(project, dataset):
        path = ds_dir / "views" / f"{view.name}.sql"
        is_forced = force and _should_process(path, force_files)
        action = resolve(view.modified, path, force=is_forced)
        if action == FetchAction.SKIP:
            logger.debug("SKIP view %s", view.name)
            continue
        if action == FetchAction.WARN:
            continue
        logger.info("FETCH view %s -> %s", view.name, path)
        if not dry_run:
            write_view_sql(path, view)

    # Routines
    for routine in bq_client.list_routines(project, dataset):
        path = ds_dir / "routines" / f"{routine.name}.sql"
        is_forced = force and _should_process(path, force_files)
        action = resolve(routine.modified, path, force=is_forced)
        if action == FetchAction.SKIP:
            logger.debug("SKIP routine %s", routine.name)
            continue
        if action == FetchAction.WARN:
            continue
        logger.info("FETCH routine %s -> %s", routine.name, path)
        if not dry_run:
    ...
    for ext in bq_client.list_external_tables(project, dataset):
        path = ds_dir / "externals" / f"{ext.name}.yaml"
        is_forced = force and _should_process(path, force_files)
